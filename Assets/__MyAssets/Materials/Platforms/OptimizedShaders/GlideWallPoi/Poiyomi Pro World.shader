Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/d8bc5fb814ebe80418de64240cfeb93e"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.1.138</color>", Float) = 0
 ...
 #define PROP_EMISSIONMASK1 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(7.377209,6.145014,0,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(7.377209,6.145014,0,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(7.377209,6.145014,0,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(1,0,0,0)) * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(13.14939,7.878154,0,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(13.14939,7.878154,0,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(13.14939,7.878154,0,1).rgb, float(0));
				#endif
				if (float(1))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE1) || !defined(OPTIMIZER_ENABLED)
						emissionStrength1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos1, float4(-10,0,0,0)) * float(5)) + _Time.x * float(2));
						#endif
					}
					else
					{
						emissionStrength1 *= calculateScrollingEmission(float4(-10,0,0,0), float(2), float(5), float(5), float(0), pos1);
					}
				}
				if (float(0))
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.9673519,1,0,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.9673519,1,0,1).a;
 ...
 #define PROP_EMISSIONMASK1 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				meta.Emission = poiFragData.emission * float(0.1) * poiMods.globalEmission;
