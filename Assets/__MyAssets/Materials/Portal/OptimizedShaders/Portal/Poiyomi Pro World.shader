Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/1c25ebf6330ba40468c3072080437812"
{
	Properties
	{
 ...
 #define PROP_IRIDESCENCERAMP 
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, cubeMap.rgb, float(1) * CubeMapMask * cubeMap.a);
				poiFragData.baseColor.rgb *= lerp(1, cubeMap.rgb, float(0) * CubeMapMask * cubeMap.a);
				poiFragData.baseColor.rgb += cubeMap.rgb * float(0) * CubeMapMask * cubeMap.a;
				poiFragData.emission += cubeMap.rgb * float(0.56) * CubeMapMask * cubeMap.a;
			}
			#endif
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,2.608238,0.8116204,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(6.79)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,2.608238,0.8116204,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,2.608238,0.8116204,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				if (float(1))
				{
					emissionStrength0 *= calculateBlinkingEmission(float(0.5), float(1.5), float(4), float(0));
				}
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(3,3,0,0)) + _Time.x * float4(5,-5,0,0)).r;
				#else
